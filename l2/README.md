...
- [x] [Transformers, the tech behind LLMs by 3Blue1Brown](https://www.youtube.com/watch?v=wjZofJX0v4M&t=183s) - 0.5h - Key takeaway: Transformers work by representing text as vectors and using attention to let tokens share information, repeatedly processing through layers to predict the next token, with weights defining the model and softmax shaping predictions into probabilities.
- [x] [Attention in transformers, step-by-step by 3Blue1Brown](https://www.youtube.com/watch?v=eMlx5fFNoYc&list=PLZHQObOWTQDNU6R1_67000Dx_ZCJB-3pi&index=7) - 0.5h - Key takeaway: Attention lets each token look at others through query vectors, using masking to keep the order, and multi-headed attention updates embeddings so the model captures richer relationships.
- [x] [How might LLMs store facts by 3Blue1Brown](https://www.youtube.com/watch?v=9-Jl0dxWQs8&t=367s) - 0.5h - Key takeaway: LLMs use multilayer perceptrons, where linear transformations with biases and nonlinear steps like ReLU are stacked, allowing the model to store and transform information.
